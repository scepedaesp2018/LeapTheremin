...
char constrain(float number, int max, int min){
    // Function saturation qui transforme aussi de float à uint_8t
    if(number>max)
        return max;
    if(number<min)
        return min;
...
int main(int argc, char** argv) {
    ...
    int deviceID;
    int arg; // auxilier pour la configuration du dispositif de reproduction
    int status; // 1 configuration acceptee
                // -1 configuration non acceptee 
                
    int t, i; // usee pour les boucles

    // Generer le tableau de basse pour sous-échantilloner avec les caracteristiques décris:
    // Voir chapitre 2 page 3
    int brightness = 128; // 0 - 255
    int waveform = 128; // 0 - 255
  ...
  Controller controller; // controller == Generer evenements et recoit des mesures
  controller.addListener(listener);
  
  // comme c'est boucle ifinite, il faut arreter directement le proces avec crtl+z
  while(true){
    // Generation du son à frequence f et amplitude ampl
    // voire chapitre 4 page 15 du rapport
  ...
  // effacer le listener apres l'execution
